...
class JurnalPKL extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view("peserta/jurnalpkl/listjurnalpkl", $data);
    }
    ...
    public function tambahjurnalpkl()
    {
        ...
        if ($validation->run()) {
            ...
            redirect('peserta/JurnalPKL');
        }
        $data['title'] = 'Add New Data Logbook ';
        $data['mapel'] = $this->jurnalpkl_model->getMapel();
        $data['pengajuanpkl'] = $this->permohonanpkl_model->getAll();
        $this->load->view("peserta/jurnalpkl/tambahjurnalpkl", $data);
    }
    ...
    {
        if (!isset($id)) redirect('peserta/JurnalPKL');
        $jurnalpkl = $this->jurnalpkl_model;
        $validation = $this->form_validation;
        $validation->set_rules($jurnalpkl->rules());
        ...
        $this->load->view("peserta/jurnalpkl/ubahjurnalpkl", $data);
    }
    ...
    public function cetak_jurnal_pkl($id_peserta = null)
    {
        $this->load->library('pdf');
        $data['jurnal_pkl'] = $this->jurnalpkl_model->getData();
        $data['data_jurnal_pkl'] = $this->jurnalpkl_model->getById($id_peserta);
        if (!$data["jurnal_pkl"]) show_error('Tidak ditemukan data', '404', 'Tidak dapat mencetak laporan jurnal PKL');
        $this->load->view('peserta/jurnalpkl/cetakjurnalpkl', $data);
    }
    ...
    public function cetak_dokumentasi_jurnal($id_peserta = null)
    {
        ...
        $this->load->view('peserta/jurnalpkl/cetakdokumentasi', $data);
    }
}
